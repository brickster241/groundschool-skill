...
export function GlossaryPage() {
  ...
  const terms = useMemo(() => {
    ...
    const sorted = [...c.glossary].sort((a, b) => a.term.localeCompare(b.term))
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8 md:py-12" key={c.branch}>
      <p className="placard">Vocabulary · {c.branch}</p>
      ...
      <div className="mt-5 flex items-center gap-2 rounded-lg border border-line bg-panel px-3">
        ...
        <input
          ...
          placeholder={`Search ${c.glossary.length} terms…`}
      ...
      <dl className="mt-6 divide-y divide-line/60">
        {terms.map((t) => {
          const track = t.trackId ? c.trackById.get(t.trackId) : undefined
          return (
